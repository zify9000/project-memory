...
- 每次任务收口、准备向用户汇报完成前 → ① 刷新 `PROGRESS.md`（覆写=刷新快照：保留未解决坑、摘除已完成已提交、未合并项标"待合并"）；② 回看本次改动是否漏触发上面任一条规则（尤其建了新目录/改了入口 → `ARCHITECTURE.md`、改了模块 → `impl/`、拍板了 → ADR 草稿），漏了当场补上
